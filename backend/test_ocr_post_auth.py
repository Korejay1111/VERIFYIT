...
try:
    r = requests.post(BASE + '/auth/register', json=reg_payload, timeout=10)
    if r.status_code not in (200,201):
        # If user exists, try login
        login_payload = {'username': reg_payload['username'], 'password': reg_payload['password']}
        r2 = requests.post(BASE + '/auth/login', json=login_payload, timeout=10)
    ...
    r3 = requests.post(BASE + '/extract-text', json=payload, headers=headers, timeout=30)
